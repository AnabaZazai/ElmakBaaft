...
<div class="flex justify-center items-center ">
    ...
    <form action="/websitecontactus" method="Post" id="ContactForm">
      ...
      @csrf
		<div class="w-full p-8 my-4 md:px-12 lg:w-9/12 lg:pl-20 lg:pr-40 mr-auto rounded-2xl shadow-2xl">
            ...
            type="text" name="first_name" placeholder="First Name" data-parsley-pattern="[a-zA-Z ]+$" data-parsley-trigger="keyup" />
              @error('first_name')
              <span class="text-red-600 text-sm"> {{ $message }}</span>
              @enderror
				<input class="w-full  text-gray-900 mt-2 p-3 rounded-lg focus:outline-none focus:shadow-outline"
            type="text" name="last_name" placeholder="Last Name" data-parsley-pattern="[a-zA-Z ]+$" data-parsley-trigger="keyup" />
              @error('last_name')
              <span class="text-red-600 text-sm"> {{ $message }}</span>
              @enderror

				<input class="w-full  text-gray-900 mt-2 p-3 rounded-lg focus:outline-none focus:shadow-outline"
            type="email" name="email" placeholder="Email"  data-parsley-type="email" data-parsley-trigger="keyup" />
              @error('email')
              <span class="text-red-600 text-sm"> {{ $message }}</span>
              @enderror
              
				<input class="w-full  text-gray-900 mt-2 p-3 rounded-lg focus:outline-none focus:shadow-outline"
            type="number" name="contact_num" placeholder="Phone" data-parsley-pattern="[0-9]+$" data-parsley-length="[10-13]+$"  data-parsley-trigger="keyup" />
              @error('contact_num')
              <span class="text-red-600 text-sm"> {{ $message }}</span>
              @enderror
        </div>
